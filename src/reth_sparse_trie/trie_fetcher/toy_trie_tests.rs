...
fn fetch_multiproof() {
    ...
    let (got_root_hash, nodes_updated) = sparse_trie.root_hash_advanced(false).expect("must hash");

    assert_eq!(nodes_updated, 1);
    assert_eq!(got_root_hash, reference_root_hash);

    let (got_root_hash, nodes_updated) = sparse_trie.root_hash_advanced(true).expect("must hash");
